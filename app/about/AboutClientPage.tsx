        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
              ...
              <p className="text-xl text-dark/60 font-light leading-relaxed max-w-lg mb-10">
            ...
            {categories.map((cat, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }}>
                <Link href={cat.href} className="group block bg-white p-8 rounded-3xl border border-cream-300 hover:shadow-xl hover:shadow-primary/5 hover:border-primary/50 transition-all duration-300 h-full flex flex-col">
                  ...
                  <p className="text-dark/60 leading-relaxed flex-1 mb-8">{cat.description}</p>
            ...
            <div className="grid sm:grid-cols-2 gap-6">
              {valueProps.map((prop, idx) => (
                <motion.div key={idx} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }}
                  ...
                  <p className="text-white/60 text-sm leading-relaxed">{prop.desc}</p>
                </motion.div>
              ))}
            </div>
          </div>
